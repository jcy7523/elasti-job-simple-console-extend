...
public final class WwwAuthFilter implements Filter {
   ...
   // private String guestPassword;
    ...
    public void init(final FilterConfig filterConfig) throws ServletException {
       ...
       Set<Object> userNameSet = props.keySet();
       for(Object userName : userNameSet){
    	   String passwordAndPrivlege = (String)props.get(userName);
    	   String password = "";
    	   String privlege = "";
    	   if(org.apache.commons.lang3.StringUtils.isNotEmpty(passwordAndPrivlege)){
    		   String[] passwordAndPrivlegeArr = passwordAndPrivlege.split(",");
    		   if(passwordAndPrivlegeArr.length == 2){
    			   password = passwordAndPrivlegeArr[0];
    			   privlege = passwordAndPrivlegeArr[1];
    		   }else{
    			   // 没有设置权限
    			   password = passwordAndPrivlege;
    		   }
    	   }
    	   userMap.put( (String)userName  + ":" + password, new User((String)userName, privlege));
       }
    ...
    public void doFilter(final ServletRequest request, final ServletResponse response, final FilterChain chain) throws IOException, ServletException {
        ...
        if (null != authorization && authorization.length() > AUTH_PREFIX.length()) {
            ...
            User user = userMap.get(new String(Base64.decodeBase64(authorization)));
            
            if (user != null) {
            	authenticateSuccess(httpRequest, httpResponse, user);
    ...
    private void authenticateSuccess(final HttpServletRequest request, final HttpServletResponse response, final User user) {
    	boolean isGuest = !ROOT_DEFAULT_USERNAME.equals(user.getUsername());
