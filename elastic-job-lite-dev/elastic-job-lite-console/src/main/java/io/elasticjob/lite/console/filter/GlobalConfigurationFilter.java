...
public final class GlobalConfigurationFilter implements Filter {
    ...
    private void loadActivatedRegCenter(final HttpSession httpSession) {
        ...
        if (config.isPresent()) {
            ...
            boolean isConnected = setRegistryCenterNameToSession(regCenterService.find(configName, regCenterService.loadAll((User)httpSession.getAttribute("username"))), httpSession);
            if (isConnected) {
