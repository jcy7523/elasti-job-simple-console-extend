...
public final class RegistryCenterRestfulApi {
    ...
    public Collection<RegistryCenterConfiguration> load(final @Context HttpServletRequest request) {
        ...
        return regCenterService.loadAll((User)request.getSession().getAttribute("username")).getRegistryCenterConfiguration();
    }
    ...
    public boolean connect(final RegistryCenterConfiguration config, final @Context HttpServletRequest request) {
        boolean isConnected = setRegistryCenterNameToSession(regCenterService.find(config.getName(), regCenterService.loadAll((User)request.getSession().getAttribute("username"))), request.getSession());
        if (isConnected) {
